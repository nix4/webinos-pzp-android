...
var AndroidInit = function(settings){
    ...
    this.monitorSetings = settings.MONITOR_SETTINGS;
...
AndroidInit.prototype.selectComponents = function(components, cb){
    ...
    for(var i in components){
        ...
        if(components[i] !== undefined){
            ...
            if(cmp.indexOf("API " + this._apiLevel) > -1 || cmp.indexOf('SDK Platform-tools') > -1
                // Due to changes in android r22 that requires user o accept license,
                // we are no going to upgrade the SDK tools
                //|| cmp.indexOf('SDK Tools') > -1
                || cmp.indexOf('Build-tools') > -1 ) {
                if(cmp.indexOf('Build-tools') > -1 ) console.log("************************Build tools were found");
