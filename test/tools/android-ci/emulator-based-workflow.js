...
var AndroidEmulatorBasedCIWorkflow = function(androidCI){
    ...
    emulator.on("Ready", function(port){
        ...
        //if(status == 0){
        async.series({
            appInstall: function(callback){
                installApps(port, callback);
            },
            appExec: function(callback){
                execNMonitorPZP(port, androidCI.monitorSetings, callback);
            }
    ...
    function execNMonitorPZP(port, settings, cb){
        var monitor_runner = require("./run-monitor");
	var waitTime = 5000;
        new monitor_runner("emulator-" + port, waitTime, settings, function(code){
            if(code !== 0){
		if(code === 199)
                	cb(new Error("A timeout error occurred while running monitoring the PZP. This is because PZP initialization took too more than " + waitTime + "miliseconds"));
                else
			cb(new Error("An error occurred while running the PZP with code:" + code));
}
            else
                cb(null);
        });
    ...
    function installApps(port, cb) {
        ...
        async.series({
            ...
            if (!err) {
                ...
                cb(null, "Finished Installing Apps. Exiting with success!");
            }  else{
                cb(new Error("Error occurred while installing app: " + err), "");
            }
        });
    }
    ...
    function androidPrep(callback){
        ...
        androidCI.downloadAndroidSDK(function(status, outputFile){
            ...
            //the outputfile is received as an array forwarded by the downloadAndroidSDK
        ...
        async.parallel({
            ...
            function(err, results){
                if(!err){
                    ...
                    cb(new Error("Emulaor Preparation Failed."));
                }
            }
        );
